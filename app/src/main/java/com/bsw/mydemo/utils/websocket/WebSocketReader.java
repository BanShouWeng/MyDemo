...
import com.bsw.mydemo.utils.websocket.utils.Utf8Validator;
...
class WebSocketReader extends Thread {
    ...
    private Map<String, String> parseHttpHeaders(String[] httpResponse) throws UnsupportedEncodingException {
        ...
        for (String line : httpResponse) {
            if (line.length() > 0) {
                ...
                if (h.length == 2) {
                    headers.put(h[0], h[1]);
                    Logger.d(getName(),String.format("'%s'='%s'", h[0], h[1]));
                }
            }
        }
    ...
    private Pair<Integer, String> parseHttpStatus(String statusLine) throws UnsupportedEncodingException {
        ...
        Logger.d(getName(),String.format("Status: %d (%s)", statusCode, statusMessage));
        ...
        try {
            do {
                ...
                if (len > 0) {
                    ...
                    Logger.d(getName(),"run() : ConnectionLost");
            ...
            Logger.d(getName(),"run() : WebSocketException (" + e.toString() + ")");
            ...
            Logger.d(getName(),"run() : Exception (" + e.toString() + ")");
